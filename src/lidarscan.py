...
def get_coordinates(msg):
	...
	goal_x = dist*np.sin(theta) - 0.08
	goal_y = dist*np.cos(theta) - 0.2
...
def action(msg):
	...
	while not rospy.is_shutdown():
		if abs(coords[2] - theta) > 0.08:
			speed.linear.x = 0
			speed.angular.z = -3.141
			vel.publish(speed)
			rospy.loginfo("Aligning angle")
		elif abs(coords[2] - theta) < 0.08 and msg.ranges[359] > 0.4 and msg.ranges[369] > 0.4 and msg.ranges[349] > 0.4:
			#align == True
			speed.linear.x = -0.7
			speed.angular.z = 0
			vel.publish(speed)
			rospy.loginfo("Moving forward")
		elif align and msg.ranges[359] < 0.4 and msg.ranges[369] < 0.4 and msg.ranges[349] < 0.4:
			speed.linear.x = 0
			speed.angular.z = 0
			vel.publish(speed)
